...
$(function(){
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $.each(data, function(index, product) {
          productsList.append('<div class="product"><h3>' + product.name + '</h3><p>' + product.description + '</p><p class="price">$' + product.price + '</p><button class="add-to-cart" data-id="' + product.id + '">Add to Cart</button></div>');
        });
      },
      error: function() {
  ...
  function displayCart() {
    ...
    $.each(cart, function(index, item) {
      total += item.price;
      cartItems.append('<div class="cart-item"><h3>' + item.name + '</h3><p>' + item.description + '</p><p class="price">$' + item.price + '</p><button class="remove-item" data-index="' + index + '">Remove</button></div>');
    });
